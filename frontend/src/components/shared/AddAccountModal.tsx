...
import { GroupamaEsPanel } from '@/components/sync/GroupamaEsPanel'
...
type WizardStep = 'selector' | 'banks' | 'exchanges' | 'wallets' | 'tr' | 'bourseDirect' | 'groupamaEs' | 'finary' | 'manual'
...
const SOURCES: { key: WizardStep; icon: typeof Landmark; labelKey: string; descKey: string }[] = [
  ...
  { key: 'groupamaEs', icon: PiggyBank, labelKey: 'sync.groupamaEs.title', descKey: 'addAccount.desc.groupamaEs' },
...
export function AddAccountModal({ open, onOpenChange }: AddAccountModalProps) {
  ...
  async function handleManualSubmit(data: {
    name: string
    type: 'LEP' | 'PEA' | 'COMPTE_TITRES' | 'PEE' | 'PERCOL' | 'CRYPTO' | 'CHECKING' | 'SAVINGS' | 'REAL_ESTATE' | 'LOAN' | 'OTHER'
    provider?: string
    currency: string
        ...
        <DialogContent className="max-w-xl">
          ...
          <>
              ...
              {step === 'groupamaEs' && (
                <>
                  <BackButton onClick={() => setStep('selector')} />
                  <GroupamaEsPanel onConnected={handleDone} />
                </>
              )}
                ...
                {mappings[index]?.action === 'CREATE_NEW' && mappings[index].newAccount && (
                  <div className="grid gap-2 sm:grid-cols-2">
                    ...
                    <div className="space-y-1">
                      ...
                      >
                        {(['CHECKING', 'SAVINGS', 'LEP', 'PEA', 'COMPTE_TITRES', 'PEE', 'PERCOL', 'CRYPTO', 'REAL_ESTATE', 'LOAN', 'OTHER'] as const).map((type) => (
